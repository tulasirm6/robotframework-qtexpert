...
```bash
# Build the official Red Hat Enterprise Linux 9 (RHEL 9 / UBI 9) image
docker build -t qtexpert-rhel9 .

# Run the RHEL 9 container with headless Xvfb and VNC
docker run -d --name qtexpert-rhel9 -p 5900:5900 -v $(pwd):/workspace qtexpert-rhel9

# Run Robot Framework test suites inside RHEL 9
